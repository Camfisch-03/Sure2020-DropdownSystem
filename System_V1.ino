...
//make a backup section so if power goes out it can pick up where it left off
...
bool begun = false, dropped = false, newData = false, useGPS = true, emergency_Status = false, pass = false;
...
uint32_t utcTime;
int addr = 0; // occasionally increment this to avoid dammage to the teensy
...
void setup() {  //Setup=-=-=-=-=-=-=-=-=-==-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  if(EEPROM.read(addr) != 0){
    currentTime[6] = -1 * (EEPROM.read(addr)*30000L); // negative to add to millis rather than detract
...
void loop() {////main loop=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  while (1 == 1) { // primary functionality, once flight begins =-=-=-=-=-=-=-=
    ...
    if (FlightTime() % 60000UL == 59999UL && !pass && FlightTime() <= 60 * 60 * 1000UL) { // runs once every minute up till 1 hour
      saveTime(); // idk yet
      pass = true;
    } //end if
    else if ( FlightTime() % 60000UL == 0) { // should make the loop above run once and only once per 5 second interval.
      pass = false;
...
void emergency_System() { // handes various situations classified as emergencies =-=-=-=-=-=-=
  ...
  //Serial.println("7");
  if (millis() - currentTime[6] >= overFlowTime || emergency_Status) { // if flight time has exceeded the max desired time, or an emergency is already declared
    if (!useGPS && !dropped) { // GPS is givinga bad signal and dropsondes havent been dropped yet. (shouldnt technically reach here, but still)
      ...
      if (GPSdata[Alt] >= GPSdata[prevAlt]) { // were still going up
        ...
        if (GPSdata[Alt] >= 10000) { // but were high enough to drop and still get some data
          ...
          overFlowTime += 60 * 1000UL; // hope this is a mistake and just wait a bit. maybe something will change??
  ...
  if (false) { // something else? idk yet
...
void saveTime() { // save the FlightTime for possible later use
  
  EEPROM.write(addr, ((int)(FlightTime() / 30000UL))); // divide by 30000 because EEPROM can only store 1 byte. this means add 1 every 30 seconds from the beginning of the flight.
